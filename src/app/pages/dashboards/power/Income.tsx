...
export function Income() {
  // Process transactions data for Banking sector (sector_id: 1)
  const bankingData = useMemo(() => {
    // Filter transactions for Banking sector
    ...
    const totalVatIncome = bankingTransactions.reduce((total, transaction) => {
    ...
    <Card className="overflow-hidden">
      ...
      <div className="px-4 sm:px-5">
        <p className="dark:text-dark-100 text-xl font-semibold text-gray-800">
          {formatCurrency(bankingData.totalVatIncome)}
        </p>
      </div>
      <div className="ax-transparent-gridline">
        <Chart
          options={chartConfig}
          series={bankingData.series}
          type="area"
          height={150}
        />
      </div>
