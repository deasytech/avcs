...
export interface PowerBusiness {
  id: number;
  name: string;
  rating: number;
  review_count: number;
...
export function usePowerData() {
  const powerData = useMemo(() => {
    ...
    const sector3Transactions = transactionsData.filter(t => t.sector_id === 3);
    ...
    sector3Transactions.forEach(transaction => {
    ...
    // Get business details for sector 3
    const sector3Businesses = businessesData.filter(b => b.sector_id === 3);

    // Combine business details with performance data
    const businessData: PowerBusiness[] = sector3Businesses.map(business => {
