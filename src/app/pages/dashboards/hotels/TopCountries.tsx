...
export function TopCountries() {
  // Get hotels businesses (sector_id: 4)
  const hotelsBusinesses = businessesData.filter(
    (business) => business.sector_id === 4
  );

  // Calculate stats for each hotels business
  const businessStats: BusinessStats[] = hotelsBusinesses.map((business) => {
    const businessTransactions = transactionsData.filter(
      (transaction) => transaction.business_id === business.id && transaction.sector_id === 4
    );
  ...
  // Count total hotels transactions
  const hotelsTransactionCount = transactionsData.filter(
    (transaction) => transaction.sector_id === 4
  ).length;
  ...
  return (
    <Card className="px-4 pb-5 sm:px-5">
      ...
      <div>
        <p>
          <span className="dark:text-dark-100 text-2xl text-gray-800">{hotelsTransactionCount}</span>
        </p>
