...
// Process transaction data for hotels sector (sector_id: 4)
const processTransactionData = () => {
  ...
  // Filter transactions for hotels sector only
  const hotelsTransactions = transactionsData.filter(transaction => transaction.sector_id === 4);
...
// Calculate overview metrics for hotels sector (sector_id: 4)
const calculateOverviewMetrics = () => {
  // Filter transactions for hotels sector only
  const hotelsTransactions = transactionsData.filter(transaction => transaction.sector_id === 4);

  const allTransactions = hotelsTransactions.reduce((sum, transaction) =>
    sum + parseNairaAmount(transaction.transaction_amount), 0);
  const totalChargeable = hotelsTransactions.reduce((sum, transaction) =>
    sum + parseNairaAmount(transaction.transaction_amount_chargeable), 0);
  const totalVat = hotelsTransactions.reduce((sum, transaction) =>
    sum + parseNairaAmount(transaction.transaction_amount_vat), 0);
  const totalVolume = hotelsTransactions.length; // Count of transactions
