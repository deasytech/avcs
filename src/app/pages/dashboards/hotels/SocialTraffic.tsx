...
export function SocialTraffic() {
  const { channels, totalRevenue, growthPercentage } = useMemo(() => {
    ...
    const hotelsTransactions = transactionsData.filter(transaction => {
      const business = businessesData.find(b => b.id === transaction.business_id);
      return business?.sector_id === 4;
    });

    // Calculate total revenue per business for hotels only
    const businessRevenue = new Map<number, number>();

    hotelsTransactions.forEach(transaction => {
    ...
    // Calculate growth percentage (simulated based on transaction dates) for hotels only
    const currentMonth = new Date().getMonth();
    const currentMonthHotelsTransactions = hotelsTransactions.filter(t =>
      new Date(t.transaction_date).getMonth() === currentMonth
    );
    const previousMonthHotelsTransactions = hotelsTransactions.filter(t =>
      new Date(t.transaction_date).getMonth() === currentMonth - 1
    );

    const currentMonthRevenue = currentMonthHotelsTransactions.reduce((sum, t) =>
      sum + parseFloat(t.transaction_amount.replace(/[₦,]/g, '')), 0
    );
    const previousMonthRevenue = previousMonthHotelsTransactions.reduce((sum, t) =>
      sum + parseFloat(t.transaction_amount.replace(/[₦,]/g, '')), 0
    );
