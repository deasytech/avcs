...
// Process transaction data for banking sector (sector_id: 1)
const processTransactionData = () => {
  ...
  // Filter transactions for banking sector only
  const bankingTransactions = transactionsData.filter(transaction => transaction.sector_id === 1);
...
// Calculate overview metrics for banking sector (sector_id: 1)
const calculateOverviewMetrics = () => {
  // Filter transactions for banking sector only
  const bankingTransactions = transactionsData.filter(transaction => transaction.sector_id === 1);

  const allTransactions = bankingTransactions.reduce((sum, transaction) =>
  ...
  const totalVat = bankingTransactions.reduce((sum, transaction) =>
    sum + parseNairaAmount(transaction.transaction_amount_vat), 0);
  const totalVolume = bankingTransactions.length; // Count of transactions
