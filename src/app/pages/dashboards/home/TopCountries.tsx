...
export function TopCountries() {
  // Get banking businesses (sector_id: 1)
  const bankingBusinesses = businessesData.filter(
    (business) => business.sector_id === 1
  );

  // Calculate stats for each banking business
  const businessStats: BusinessStats[] = bankingBusinesses.map((business) => {
    const businessTransactions = transactionsData.filter(
      (transaction) => transaction.business_id === business.id && transaction.sector_id === 1
    );
  ...
  // Count total banking transactions
  const bankingTransactionCount = transactionsData.filter(
  ...
  return (
    <Card className="px-4 pb-5 sm:px-5">
      ...
      <div>
        <p>
          <span className="dark:text-dark-100 text-2xl text-gray-800">{bankingTransactionCount}</span>
        </p>
