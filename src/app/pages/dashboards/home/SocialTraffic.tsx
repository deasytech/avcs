...
export function SocialTraffic() {
  const { channels, totalRevenue, growthPercentage } = useMemo(() => {
    ...
    const bankingTransactions = transactionsData.filter(transaction => {
      const business = businessesData.find(b => b.id === transaction.business_id);
      return business?.sector_id === 1;
    });

    // Calculate total revenue per business for banking only
    ...
    // Calculate growth percentage (simulated based on transaction dates) for banking only
    const currentMonth = new Date().getMonth();
    const currentMonthBankingTransactions = bankingTransactions.filter(t =>
      new Date(t.transaction_date).getMonth() === currentMonth
    );
    const previousMonthBankingTransactions = bankingTransactions.filter(t =>
      new Date(t.transaction_date).getMonth() === currentMonth - 1
    );

    const currentMonthRevenue = currentMonthBankingTransactions.reduce((sum, t) =>
      sum + parseFloat(t.transaction_amount.replace(/[₦,]/g, '')), 0
    );
    const previousMonthRevenue = previousMonthBankingTransactions.reduce((sum, t) =>
      sum + parseFloat(t.transaction_amount.replace(/[₦,]/g, '')), 0
    );
