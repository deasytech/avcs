...
export function SocialTraffic() {
  const { channels, totalRevenue, growthPercentage } = useMemo(() => {
    ...
    const telecomsTransactions = transactionsData.filter(transaction => {
      const business = businessesData.find(b => b.id === transaction.business_id);
      return business?.sector_id === 2;
    });

    // Calculate total revenue per business for telecoms only
    ...
    // Calculate growth percentage (simulated based on transaction dates) for telecoms only
    const currentMonth = new Date().getMonth();
    const currentMonthTelecomsTransactions = telecomsTransactions.filter(t =>
      new Date(t.transaction_date).getMonth() === currentMonth
    );
    const previousMonthTelecomsTransactions = telecomsTransactions.filter(t =>
      new Date(t.transaction_date).getMonth() === currentMonth - 1
    );

    const currentMonthRevenue = currentMonthTelecomsTransactions.reduce((sum, t) =>
      sum + parseFloat(t.transaction_amount.replace(/[₦,]/g, '')), 0
    );
    const previousMonthRevenue = previousMonthTelecomsTransactions.reduce((sum, t) =>
      sum + parseFloat(t.transaction_amount.replace(/[₦,]/g, '')), 0
    );
